...
class Penta_3cnn(nn.Module):
    def __init__(self, NN_params, vocab_lens, n_labels, start_dim_dense, device, kernel_sizes=[3,4,5,6,7]):
        ...
        self.device = device
        if NN_params['FAKE']:
            self.embed_FAKE, self.cnn_FAKE1, self.cnn_FAKE2, self.cnn_FAKE3 = self.make_layers(vocab_lens['FAKE'], kernel_sizes)
            dim_dense += self.cnn_out_size * (len(kernel_sizes))
        if NN_params['SQ']:
            self.embed_SQ, self.cnn_SQ1, self.cnn_SQ2, self.cnn_SQ3 = self.make_layers(vocab_lens['SQ'], kernel_sizes)
            dim_dense += self.cnn_out_size * (len(kernel_sizes))
        if NN_params['DVMA']:
            self.embed_DVMA, self.cnn_DVMA1, self.cnn_DVMA2, self.cnn_DVMA3 = self.make_layers(vocab_lens['DVMA'], [3])
            dim_dense += self.cnn_out_size
        if NN_params['DVSA']:
            self.embed_DVSA, self.cnn_DVSA1, self.cnn_DVSA2, self.cnn_DVSA3 = self.make_layers(vocab_lens['DVSA'], [3])
            dim_dense += self.cnn_out_size
        if NN_params['DVEX']:
            self.embed_DVEX, self.cnn_DVEX1, self.cnn_DVEX2, self.cnn_DVEX3 = self.make_layers(vocab_lens['DVEX'], [3])
            dim_dense += self.cnn_out_size
        if NN_params['DVL2']:
            self.embed_DVL2, self.cnn_DVL21, self.cnn_DVL22, self.cnn_DVL23 = self.make_layers(vocab_lens['DVL2'], [3])
            dim_dense += self.cnn_out_size
