...
class Penta_NN(nn.Module):
    def __init__(self, NN_params, vocab_lens, n_labels, start_dim_dense, kernel_sizes=[3,4,5,6,7]):
        ...
        cnn_out_size = 100
        if NN_params['FAKE']:
            self.embed_FAKE, self.cnn_FAKE1, self.cnn_FAKE2, self.cnn_FAKE3 = self.make_layers(vocab_lens['FAKE'], emb_length, cnn_out_size, kernel_sizes)
            dim_dense += cnn_out_size * (len(kernel_sizes))
        if NN_params['SQ']:
            self.embed_SQ, self.cnn_SQ1, self.cnn_SQ2, self.cnn_SQ3 = self.make_layers(vocab_lens['SQ'], emb_length, cnn_out_size, kernel_sizes)
            dim_dense += cnn_out_size * (len(kernel_sizes))
        if NN_params['DVMA']:
            self.embed_DVMA, self.cnn_DVMA1, self.cnn_DVMA2, self.cnn_DVMA3 = self.make_layers(vocab_lens['DVMA'], emb_length, cnn_out_size, [3])
            dim_dense += cnn_out_size
        if NN_params['DVSA']:
            self.embed_DVSA, self.cnn_DVSA1, self.cnn_DVSA2, self.cnn_DVSA3 = self.make_layers(vocab_lens['DVSA'], emb_length, cnn_out_size, [3])
            dim_dense += cnn_out_size
        if NN_params['DVEX']:
            self.embed_DVEX, self.cnn_DVEX1, self.cnn_DVEX2, self.cnn_DVEX3 = self.make_layers(vocab_lens['DVEX'], emb_length, cnn_out_size, [3])
            dim_dense += cnn_out_size
        if NN_params['DVL2']:
            self.embed_DVL2, self.cnn_DVL21, self.cnn_DVL22, self.cnn_DVL23 = self.make_layers(vocab_lens['DVL2'], emb_length, cnn_out_size, [3])
            dim_dense += cnn_out_size
        ...
        self.dense3 = nn.Linear(256, n_labels)
...
def _train(model, NN_params, train_generator, val_generator, save_path, n_epochs, epochs_stop=50):
...
def NN_classification(dataset, NN_params, kfold, dataset_name, n_sent, pickle_path):
    ...
    if method_name in df:
        ...
        for i, (trval_index, test_index) in enumerate(kfold.split(data, authors_labels)):
            ...
            dataset = NN_DataLoader(x_trval, x_trval_cltk, x_te, x_te_cltk, y_trval, y_te, NN_params, batch_size=100)
